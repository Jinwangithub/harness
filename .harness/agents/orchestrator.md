...
执行动作:
  ...
  6. **记录教训**：如果审查发现 Critical/Must-Fix 问题，立即写入 `.harness/memory/lessons-learned.md`，避免同类问题再次出现
  7. **只问一次**：汇总后一次性展示完整结论，询问用户是否通过
...
```
入口: Phase 6 门禁通过
Skill: 读取 .harness/skills/code-review-and-quality/SKILL.md（复用其中的测试审查标准）
执行动作:
