...
**因受阻回退时**：记录 failure evidence + 根因 → 按 `flow.md` 回退路径回退 → 修复并重验证 → 触发 Memory 则立即记录 → 风险扩大则重新执行 Flow Classifier。
...
- **Dispatch**：按 `.harness/rules/flow.md` 的当前 Phase/Step 入口卡片读取 Skills，并判断是否需要补读条件性 Skills；需隔离时创建受限泳道。Skill 文件路径按 `.harness/skills/{name}/SKILL.md` 约定解析。
