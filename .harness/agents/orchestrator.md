...
3. Mechanical Gate 通过后执行 Human Approval Gate。
4. 用户确认前不得进入下一 Phase。

### 规则 2：唯一 Orchestrator + 本地 Skills
...
### 规则 3：Phase 1 和 Phase 2 的产物严格分离
...
### 规则 8：每个发现/决策必须立即记入记忆系统

发现 Agent 的错误、做出架构决策、遇到已知问题，必须立即写入 `.harness/memory/`：
- **架构决策** → `decisions.log`（背景、方案、备选、理由、后果）
- **经验教训** → `lessons-learned.md`（问题、根因、影响、修复、预防措施）
- **已知问题** → `known-issues.md`（描述、影响范围、临时方案、计划修复）

**禁止**：发现不记、记了不预防。每一条规则的来源都应是 lessons-learned 中的一条记录。
...
Mechanical Gate: understanding.md 存在且包含复述、边界、疑问点，状态为 pass|fail|blocked
...
Mechanical Gate: spec.md 存在且 tasks.md 不由 Phase 2 创建，状态为 pass|fail|blocked
...
Mechanical Gate: tasks.md 存在且任务有验收条件，状态为 pass|fail|blocked
...
执行动作:
  ...
  2. Orchestrator 调度 incremental-implementation Skill 执行增量切片实现
  3. 每完成一个子任务，Orchestrator 报告进度（如"Task 2/6 完成"）
  4. 运行 mvn clean compile（仅编译，不做测试）
  5. 写入 coding_report_v1.md
产出: 源码 + .harness/changes/{id}/coding/coding_report_v1.md
Mechanical Gate: 编译成功且 coding_report_v1.md 存在，状态为 pass|fail|blocked
Human Approval Gate: pending-human，必须问用户「编码完成，编译通过，请确认是否可以提交评审？」
```
...
执行动作:
  ...
  6. 如果审查发现 Critical/Must-Fix 问题，立即写入 `.harness/memory/lessons-learned.md`，避免同类问题再次出现
  7. 汇总后一次性展示完整结论，询问用户是否通过
...
Mechanical Gate: 三份评审报告存在，Critical=0，Must Fix=0，状态为 pass|fail|blocked
...
Mechanical Gate: 测试通过、测试数大于 0、覆盖率达标，状态为 pass|fail|blocked
...
Mechanical Gate: 测试评审报告存在，Must Fix=0，状态为 pass|fail|blocked
...
Mechanical Gate: CI 报告存在且成功，状态为 pass|fail|blocked
...
Mechanical Gate: 部署报告存在，冒烟/回滚检查完成，状态为 pass|fail|blocked
Human Approval Gate: pending-human，询问用户确认。
```
...
动作:
  ...
  2. 如有未记录的决策 → 写入 `.harness/memory/decisions.log`
  3. 如有值得团队共享的经验 → 写入 `.harness/memory/lessons-learned.md`
  4. 生成 delivery-summary.md（汇总所有交付物、质量门禁状态、架构决策）
  5. 更新 summary.md 标记"已完成"
  6. 列出变更目录下所有归档文件作为交付证据
Mechanical Gate: delivery-summary.md、summary 状态、memory 检查完成，状态为 pass|fail|blocked，evidence_paths 完整
...
| 场景 | 必须说的话 |
|------|-----------|
| Phase 1 出口 | "Mechanical Gate: pass。以上是我对需求的理解，是否正确？请确认后进入下一阶段。" |
| Phase 2 出口 | "Mechanical Gate: pass。Spec 已产出，请确认是否通过？(是/否，问题在...)" |
| Phase 3 出口 | "Mechanical Gate: pass。任务拆解完成，每个任务均可独立验证。请确认是否合理？" |
| Phase 4 出口 | "Mechanical Gate: pass。编码完成，编译通过。进度：N 个任务已完成。请确认是否可以提交评审？" |
| Phase 4 子任务完成 | "Task {i}/{total} 已完成：{任务名称}" |
| Phase 5 各 Skill | "Code Review 审查完成" / "Security Audit 审计完成" / "Performance Review 审查完成" |
| Phase 5 出口 | "Mechanical Gate: pass。三轴审查全部完成。评审摘要：[Verdict]。请确认是否通过？" |
| Phase 6 出口 | "Mechanical Gate: pass。测试已完成，全部 {N} 个通过，覆盖率 {X}%。请确认？" |
| Phase 7 出口 | "Mechanical Gate: pass。测试评审完成。请确认？" |
| Phase 8 出口 | "Mechanical Gate: pass。CI 验证完成，构建成功。请确认？" |
| Phase 9 出口 | "Mechanical Gate: pass。部署验证完成，冒烟测试通过。请确认？" |
| Phase 10 出口 | "Mechanical Gate: pass。交付完成。以下为交付物清单... 请最终确认。" |
