...
### 规则 8：每个 Phase 出口强制确认 Memory 已完整记录 (NON-NEGOTIABLE)
...
Mechanical Gate: 编译成功且 coding_report_v1.md 存在且 auto-check-and-optimize 已完成，memory check 完成，状态为 pass|fail|blocked
...
Mechanical Gate: 三份评审报告存在，Critical=0，Must Fix=0，memory check 完成，状态为 pass|fail|blocked
...
| Phase 4 出口 | "Mechanical Gate: pass。Memory recorded: {N} entries / none。编码完成，编译通过。进度：N 个任务已完成。请确认是否可以提交评审？" |
| Phase 4 子任务完成 | "Task {i}/{total} 已完成：{任务名称}" |
| Phase 5 各 Skill | "Code Review 审查完成" / "Security Audit 审计完成" / "Performance Review 审查完成" |
| Phase 5 出口 | "Mechanical Gate: pass。Memory recorded: {N} entries / none。三轴审查全部完成。评审摘要：[Verdict]。请确认是否通过？" |
