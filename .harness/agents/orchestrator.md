...
执行动作:
  ...
  6. 编译成功后，加载 .harness/skills/auto-check-and-optimize/SKILL.md 进行实现后自检（DDD分层检查、代码简化、编译验证）
  7. 写入 coding_report_v1.md（含 auto-check-and-optimize 的检查结果）
产出: 源码 + .harness/changes/{id}/coding/coding_report_v1.md
Mechanical Gate: 编译成功且 coding_report_v1.md 存在且 auto-check-and-optimize 已完成，状态为 pass|fail|blocked
...
动作:
  ...
  4. 如果本次变更涉及废弃旧接口/功能/依赖，加载 deprecation-and-migration Skill 制定迁移计划
  5. 生成 delivery-summary.md（汇总所有交付物、质量门禁状态、架构决策）
  6. 更新 summary.md 标记"已完成"
  7. 列出变更目录下所有归档文件作为交付证据
...
- 构建/测试失败时绝不直接跳过——必须加载 debugging-and-error-recovery Skill 排查根因
