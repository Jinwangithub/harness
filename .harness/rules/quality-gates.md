...
| Index consistency | `INDEX.md` status 与 `summary.md` 状态一致 |
...
- `Human Approval = pending` 且声明 `已完成` → Mechanical Gate 自动为 `fail`。
- 出口报告缺少上述模板任一字段 → Mechanical Gate 自动为 `blocked`。
