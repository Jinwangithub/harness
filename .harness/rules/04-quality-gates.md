...
| Evidence Type | 判定方式 |
|---------------|----------|
| Command | 命令退出码、stdout/stderr 确定字段 |
| File artifact | 文件存在，路径确定，必要字段存在 |
| Search check | 确定性搜索结果、禁止词为 0 |
| Review report | 报告存在，Critical/Must Fix 计数可判定 |
| Coverage/test | 测试数 > 0，覆盖率 >= 阈值 |
| Skill load | Required Skill 已加载，有记录 |
| harness-validate.sh | 输出 PASS/WARN/FAIL，退出码可判定 |

## 4. Lite-flow Gate 表

| Step | Mechanical Gate 必查 | Fresh Evidence | Human Approval |
|------|----------------------|----------------|----------------|
| L1 需求确认+简化计划 | `summary.md`、`lite_spec.md`、`checklist.md` 存在；有 `low_risk_proof`；无强制升级风险 | Command / Exit code / Output summary / Artifact path | 用户确认后进入 L2 |
...
每个 Standard Phase 出口必须逐项填写 yes/no，不得留空：
...
|-------|----------------------|----------|--------|
| 1 | `understanding.md` 存在；禁止提前创建 `spec.md`/`tasks.md` | `request_analysis/understanding.md` | CK1 |
...
| 10 | delivery summary 存在；Memory 完整 | `delivery-summary.md` | CK10 |
...
Fresh evidence 任意字段为空或填写空洞内容 → Mechanical Gate 自动为 `blocked`。
