...
- 流程顺序与 Flow/Phase 边界：`.harness/rules/02-development-workflow.md`
- Gate 判定与 canonical 状态：`.harness/rules/04-quality-gates.md`
- Artifact 路径、字段、Skill Load Record：本文件
- Skill 加载角色与触发条件：`.harness/skills/README.md`
...
Lite-flow 不扩展成 Standard-flow 产物结构。Lite-flow 不创建 `spec.md`、`tasks.md`、`coding/`、`unit_test/`、`ci_result/`、`deployment/`、`delivery-summary.md`，除非 Stop-the-Line 后升级为 Standard-flow。
...
- **Low-risk proof**: {Lite-flow 必填；Standard-flow 可写 N/A}
- **Risk flags**: {none/security/data/api/auth/payment/perf/migration/architecture/governance/deployment/unclear-requirement}
...
## 门禁状态
| Step/Phase | Mechanical Gate | Fresh evidence | Human Approval | Approval evidence | Next/Rollback |
|------------|-----------------|----------------|----------------|-------------------|---------------|
| {Step/Phase} | pass/fail/blocked | {current evidence path/command result} | pending-human/approved/rejected/not-required-by-policy | {user confirmation record or policy reason} | {next/rollback} |

## Skill Load Records
{Standard-flow 每个 Phase 必填；Lite-flow 按需记录；模板见下方}

## Memory status
- **Memory check**: {completed/pending/blocked}
- **Memory recorded**: {N} entries / none
- **Memory evidence**: {decisions.log / lessons-learned.md / known-issues.md path or none}
...
- [ ] Flow Classification includes selection basis, low-risk proof or N/A, risk flags, confirmation policy, upgrade triggers
- [ ] Current step and Resume point are current
- [ ] Gate table uses canonical Mechanical/Human Approval statuses
- [ ] Fresh evidence is listed for every completed Step/Phase
- [ ] Approval evidence is listed for every Human Approval Gate
- [ ] Skill Load Records include Required/Conditional trigger evaluation
- [ ] Memory status and evidence are current
- [ ] Completion lock is unlocked only after Completion Claim Gate passes
...
Standard-flow 每个 Phase 必须记录 Required Skills；Support/Conditional Skills 在加载、需要或触发时记录。每个 Phase/Step 入口必须记录 Required/Conditional trigger evaluation。
...
|------------|-------|------|--------------------|--------|-----------------|
| {Phase 4} | `incremental-implementation` | Required Skill | required-by-phase | loaded/blocked/not-triggered/not-required-by-policy | {读取路径、触发原因、阻塞原因或证据路径} |
| {Phase 4} | `auto-check-and-optimize` | Required Skill | required-by-phase | loaded/blocked/not-triggered/not-required-by-policy | {读取路径、Author/Self Review 证据路径} |
| {Phase 4} | `security-and-hardening` | Conditional Skill | triggered/not-triggered + {risk/evidence} | loaded/blocked/not-triggered/not-required-by-policy | {读取路径、触发依据、阻塞原因或 N/A} |
```
...
## Low-risk Proof
- Impact surface: {单模块/少量文件/纯文档等证据}
- Behavior change: {none/low-risk + reason}
- Governance/template/process change: {no；如 yes 必须升级或记录 governance risk}
- Disallowed risk flags absent: {api/db/auth/security/perf/migration/architecture/deployment/unclear-requirement}

## Upgrade Triggers
- {风险扩大、证据不足、Gate fail|blocked、Memory 无法完整记录、需要业务判断等}

## Approval Status
- Requirements + simplified plan: {pending-human/approved/rejected/not-required-by-policy}
- Evidence: {approval record or pending reason}
...
- Fresh evidence: {path/command/search result}
- Memory check: {completed/pending/blocked; Memory recorded: N entries / none}
...
- [ ] {任务 1}
  ...
  - Verification: {命令、搜索、人工确认记录或报告路径}
- [ ] {任务 2}
  - Acceptance criteria: {可验证验收条件}
  - Verification: {命令、搜索、人工确认记录或报告路径}
...
## Human Approval Gate
- Status: {pending-human/approved/rejected/not-required-by-policy}
- Evidence: {final approval record, pending reason, or policy reason}
```
...
## Mechanical Gate
- Status: {pass/fail/blocked}
- Fresh evidence: {verification_report.md, review checks, search/command results}
...
## Confirmation Policy
- Policy: batched
- Requirements + simplified plan approval: {pending-human/approved/rejected/not-required-by-policy}
- Final verification/review approval: {pending-human/approved/rejected/not-required-by-policy}
- Final approval evidence: {user confirmation record or pending reason}
```
...
- Spec success criteria mapping:
  - {对应 `spec.md` Success Criteria 条目}
- Description: {要完成的工作}
- Acceptance criteria:
  - {可验证条件}
- Verification:
...
边界说明：`tasks.md` 只能在 Standard-flow Phase 3 首次创建；Phase 1/2 不得创建或预填。为避免混淆，禁止使用 `Phase 1/2/3` 作为实现步骤标题；统一使用 `Task Group`、`Slice` 或 `Checkpoint`。每个 Task Group/Slice 必须映射到 `spec.md` Success Criteria。
