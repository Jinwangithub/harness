...
description: 代码写好后自动自检优化技能。每次完成代码编写任务后自动触发，调用 code-review-and-quality 进行五轴审查，调用 code-simplification 进行代码简化，并按项目 spec 中定义的命令执行编译验证。专为 Java DDD 四层架构项目设计，额外检查分层架构合规性。Make sure to use this skill after every code implementation task - it is not optional.
---
...
When used inside Harness Engineering, this Skill is only Harness Phase 4 Author/Self Review. Compile/build commands must come from the approved `spec.md`; do not hard-code Maven as the Harness command. This Skill does not replace Phase 5 Independent Review.
...
└─────────────────────────┘
         ...
         ▼
┌─────────────────────────┐
│ 5. Compile Verify      │  ← 执行 spec.md 中的编译命令
└─────────────────────────┘
         │
         ▼
┌─────────────────────────┐
...
```bash
cd /path/to/project
{build_or_compile_command_from_approved_spec}
```

Harness 中命令来源必须是 approved `spec.md` 的 Build/Compile 字段；非 Maven 项目不得套用 Maven 命令。
...
- [ ] 执行了 DDD 架构检查
- [ ] 执行了 approved `spec.md` 中的编译命令并通过
- [ ] 生成了 Markdown 报告
